...
# webarchive Setup Script (Ubuntu/Debian/FreeBSD/macOS)
#   - Project Homepage: https://github.com/webarchive/ArchiveBox
#   - Install Documentation: https://github.com/webarchive/ArchiveBox/wiki/Install
# Script Usage:
#    curl -fsSL 'https://raw.githubusercontent.com/webarchive/ArchiveBox/dev/bin/setup.sh' | bash
#           (aka https://docker-compose.webarchive.dev)
...
if [ $(id -u) -eq 0 ]; then
    ...
    echo "      adduser webarchive && usermod -a webarchive -G sudo && su webarchive"
...
if (which docker > /dev/null && docker pull webarchive/archivebox:latest); then
    echo "[+] Initializing an ArchiveBox data folder at ~/webarchive/data using Docker Compose..."
    mkdir -p ~/archivebox/data || exit 1
    cd ~/archivebox
    if [ -f "./index.sqlite3" ]; then
        mv -i ~/webarchive/* ~/archivebox/data/
    fi
    curl -fsSL 'https://raw.githubusercontent.com/webarchive/ArchiveBox/stable/docker-compose.yml' > docker-compose.yml
    mkdir -p ./etc
    curl -fsSL 'https://raw.githubusercontent.com/webarchive/ArchiveBox/stable/etc/sonic.cfg' > ./etc/sonic.cfg
    ...
    echo "    docker compose run archivebox add 'https://example.com' 'https://example.org'"
    echo "    docker compose run webarchive list"
    echo "    docker compose run webarchive help"
    exit 0
    ...
    cd ./data
    docker run -v "$PWD":/data -it --rm webarchive/archivebox:latest init --setup
    echo
    echo "[+] Starting ArchiveBox server using: docker run -d webarchive/archivebox..."
    docker run -v "$PWD":/data -it -d -p 8000:8000 --name=webarchive webarchive/archivebox:latest
    sleep 7
    which open > /dev/null && open "http://127.0.0.1:8000" || true
    echo
    echo "[√] Server started on http://0.0.0.0:8000 and data directory initialized in ~/archivebox/data. Usage:"
    echo "    cd ~/archivebox/data"
    echo "    docker ps --filter name=webarchive"
    echo "    docker kill archivebox"
    echo "    docker pull archivebox/archivebox"
    echo "    docker run -v $PWD:/data -d -p 8000:8000 --name=webarchive webarchive/archivebox"
    echo "    docker run -v $PWD:/data -it webarchive/webarchive manage createsuperuser"
    echo "    docker run -v $PWD:/data -it webarchive/webarchive add 'https://example.com'"
    echo "    docker run -v $PWD:/data -it webarchive/webarchive list"
    echo "    docker run -v $PWD:/data -it webarchive/webarchive help"
    exit 0
fi

echo
echo "[!] It's highly recommended to use webarchive with Docker, but Docker wasn't found."
echo
...
echo "[i] webarchive Setup Script 📦"
echo
...
echo "        https://github.com/webarchive/ArchiveBox/wiki/Install"
echo
...
if which apt-get > /dev/null; then
    echo "[+] Adding webarchive apt repo and signing key to sources..."
    if ! (sudo apt install -y software-properties-common && sudo add-apt-repository -u ppa:archivebox/archivebox); then
        echo "deb http://ppa.launchpad.net/webarchive/archivebox/ubuntu focal main" | sudo tee /etc/apt/sources.list.d/archivebox.list
        echo "deb-src http://ppa.launchpad.net/webarchive/archivebox/ubuntu focal main" | sudo tee -a /etc/apt/sources.list.d/archivebox.list
        sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys C258F79DCC02E369
        sudo apt-get update -qq
    fi
    echo
    echo "[+] Installing webarchive system dependencies using apt..."
    ...
    echo
    echo "[+] Installing webarchive python dependencies using pip3..."
...
# On Mac:
    ...
    python3 -m pip install --upgrade --ignore-installed webarchive yt-dlp playwright
else
...
if ! (python3 -m django --version && python3 -m pip show archivebox && which -a archivebox); then
    echo "[X] Django and webarchive were not found after installing!"
...
echo
echo "[+] Initializing webarchive data folder at ~/archivebox/data..."
mkdir -p ~/archivebox/data || exit 1
cd ~/archivebox
if [ -f "./index.sqlite3" ]; then
    mv -i ~/archivebox/* ~/archivebox/data/
...
echo
echo "[+] Starting webarchive server using: nohup archivebox server &..."
nohup python3 -m webarchive server 0.0.0.0:8000 > ./logs/server.log 2>&1 &
sleep 7
which open > /dev/null && open "http://127.0.0.1:8000" || true
echo
echo "[√] Server started on http://0.0.0.0:8000 and data directory initialized in ~/archivebox/data. Usage:"
...
echo "    webarchive add 'https://example.com'"              # webarchive a new URL
echo "    webarchive list                                    # see URLs archived"
echo "    webarchive help                                    # see more help & examples"
